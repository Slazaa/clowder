...
pub const PFNGLGETUNIFORMLOCATIONPROC = *const fn (program: GLuint, name: [*]const u8) callconv(.C) GLint;
...
pub var glBufferSubData: PFNGLBUFFERSUBDATAPROC = undefined;
...
pub var glUniform4f: PFNGLUNIFORM4FPROC = undefined;
...
pub fn load(loader: Loader) void {
    ...
    glGetUniformLocation = @ptrCast(loader("glGetUniformLocation"));
    glLinkProgram = @ptrCast(loader("glLinkProgram"));
    glShaderSource = @ptrCast(loader("glShaderSource"));
    glUniform1i = @ptrCast(loader("glUniform1i"));
    glUniform4f = @ptrCast(loader("glUniform4f"));
    glUniformMatrix4fv = @ptrCast(loader("glUniformMatrix4fv"));
